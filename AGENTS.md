...
This file is auto-loaded by Codex CLI at the start of every session in this repository. It is intentionally short. Full context lives in `/docs` — treat `/docs` as the single source of truth, not this file.

This file and `CLAUDE.md` (Claude Code's equivalent auto-loaded file) must stay in sync. If you update one, update the other the same way.
...
Currently: this README, this file, `CLAUDE.md`, and `/docs`. No `src/` exists yet — its proposed layout is defined in `docs/CONVENTIONS.md`; create it exactly as specified there when Phase 1 implementation begins.
